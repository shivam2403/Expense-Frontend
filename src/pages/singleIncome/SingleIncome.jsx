...
const SingleIncome = () => {
  ...
  useEffect(()=>{
    const fetchincome=async(req,res)=>{
      try {
        const res=await axios.get(`https://expense-backend-xz7w.onrender.com/api/v1/income/${id}`);
        setincome(res.data);
  ...
  const handleUpdate=async(e)=>{
    ...
    try {
      console.log(id);
      const res=await axios.put(`https://expense-backend-xz7w.onrender.com/api/v1/income/${id}`,{
        amount:amount,
        category:category,
  ...
  const handleDelete=async()=>{
    ...
    try {
      await axios.delete(`https://expense-backend-xz7w.onrender.com/api/v1/income/${id}`);
      setOpenDelete(false);
      navigate('/income');    
    } catch (error) {
      console.log(error);
