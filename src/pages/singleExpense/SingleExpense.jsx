...
const SingleExpense = () => {
  ...
  useEffect(()=>{
    const fetchExpense=async(req,res)=>{
      try {
        const res=await axios.get(`https://expense-backend-xz7w.onrender.com/api/v1/expense/${id}`);
        setExpense(res.data);
  ...
  const handleUpdate=async(e)=>{
    ...
    try {
      console.log(id);
      const res=await axios.put(`https://expense-backend-xz7w.onrender.com/api/v1/expense/${id}`,{
        amount:amount,
        category:category,
  ...
  const handleDelete=async()=>{
    ...
    try {
      await axios.delete(`https://expense-backend-xz7w.onrender.com/api/v1/expense/${id}`);
      setOpenDelete(false);
      navigate('/expense');    
    } catch (error) {
      console.log(error);
